...
var zhb=1;
  while (node = nodes.pop()) {
    ...
    // alert("第"+zhb+"次取node孩子有："+children(node.data));
  ...
  return root.eachBefore(computeHeight);//eachBefore 前序遍历，然后每个节点依次执行 function
}
...
function packEnclose(circles) {
  ...
  pack: for (i = 3; i < n; ++i) {
    ...
    // 找到前链上最接近的相交圆（如果有的话）。
    //“Closeness” is determined by linear distance along the front-chain.“亲密度”是由沿着前链的直线距离决定的。
    // “Ahead” or “behind” is likewise determined by linear distance.“前”或“后”同样由直线距离确定。
    ...
    // 计算新的最接近的圆对的质心。Compute the new closest circle pair to the centroid.
    aa = score(a);
    while ((c = c.next) !== b) {
      if ((ca = score(c)) < aa) {
        a = c, aa = ca;
  ...
  // 计算前链的封闭圆。Compute the enclosing circle of the front chain.
  a = [b._], c = b; while ((c = c.next) !== b) a.push(c._); c = enclose(a);

  // 将圆圈翻译成围绕原点的圆圈。Translate the circles to put the enclosing circle around the origin.
